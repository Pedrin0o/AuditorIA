...
MODELO_PROFESSOR_3 = "openrouter/free"  # OpenRouter - roteador automático entre modelos gratuitos disponíveis
...
def aluno_ainda_tem_duvida(resposta_aluno):
    """
    O modelo local (1B) não segue formato à risca (escreve 'Sem duvidas', '[Sem duvidas]',
    'Sem duvidas, obrigado!' etc. em vez do token exato esperado). Por isso, em vez de tentar
    adivinhar todas as variações por palavra-chave, pedimos para o Professor 1 (mais confiável
    em seguir instruções) CLASSIFICAR a mensagem. Retorna True se for uma pergunta real.
    """
    prompt = f"""Classifique a mensagem de um aluno abaixo em uma das duas categorias:

- "PERGUNTA": o aluno está fazendo uma pergunta real, pedindo mais explicação ou aprofundamento.
- "ENCERROU": o aluno está dizendo que não tem mais dúvidas, que entendeu tudo, ou está apenas
  resumindo/concordando com o que já foi dito, sem fazer nenhuma pergunta nova.

Mensagem do aluno: \"\"\"{resposta_aluno}\"\"\"

Responda APENAS com uma palavra: PERGUNTA ou ENCERROU."""
    try:
        completion = groq_client.chat.completions.create(
            model=MODELO_PROFESSOR_1,
            messages=[{"role": "user", "content": prompt}]
        )
        classificacao = completion.choices[0].message.content.strip().upper()
        return "PERGUNTA" in classificacao
    except Exception as e:
        print(f"[Aviso] Erro ao classificar a dúvida do aluno: {e}. Encerrando o tema por segurança.")
        return False
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for rodada in range(2, MAX_RODADAS_DUVIDA + 2):
            ...
            if not aluno_ainda_tem_duvida(duvida):
